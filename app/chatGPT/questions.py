class Query:
    ...
    GENERATE_TABLE = Question(
        "I will provide you with outputs from smart contract frameworks Securify, SmartCheck, Slither and Solhint. Based on the outputs from given contracts, group the common errors among the contracts by meaning and context.Do not include any explanations, do not wrapp content with ```json ```, only provide a RFC8259 compliant JSON response following this format: {error_name: list_of_frameworks_names} eg. {Incorect solidity version: [solhint, smartcheck], function call without data: [slither],} without deviation and markdown formatting. Take into considerations errors from every framework. Errors should be in human readable form. Sometimes errors have different names but mean the same, handle it, especially smartcheck - try to rename them. Frameworks are separated by the \"START\" and \"END\" keywords. START securify: %s END, START solhint: %s END, START slither: %s END, smartcheck: %s END"
    )
    
    GENERATE_DEATILED_TABLE = Question(
        "I will provide you with the outputs of Solidity contracts analyzed by the Securify, SmartCheck, Slither, and Solhint frameworks. Do not include any explanations, do not wrapp content with ```json ```,  only provide a RFC8259 compliant JSON response following this format: {error_name: list_of_frameworks_names} eg. {Incorect solidity version: [solhint, smartcheck], function call without data: [slither]} without deviation and markdown formating, trying to indicates that a particular framework detected the error. Be sure to consider detailed errors and warnings reported by each framework separately. Take into considerations errors from every framework. Frameworks are separated by the \"START\" and \"END\" keywords. START securify: %s END, START solhint: %s END, START slither: %s END, smartcheck: %s END"
    )
    # GENERATE_TABLE = Question(
    ...
    SUGGESTIONS = Question(
        "I will provide you with errors generated by smart contract test frameworks. Response should be in JSON format. Based on the outputs from given contracts prepare list of suggestions how code can be fixed. Do not include any explanations,  do not wrapp content with ```json ```,  only provide a RFC8259 compliant JSON response following this format: ({suggestion: Specify the required compiler version in the source file. code_snippet: pragma solidity ^0.5.2) without deviation and markdown formatting. Add code snippets based one contract code if it is possible. Return also proposition of fixed contract code. You should return output in JSON in form {suggestions: [{suggestion: fix suggestion it should be described well, example: add fix example(if code use markdown format) it can be nullable}], fixed_code: fixed_smartcontract_code }. This is smartcontract code: %s, these are errors: %s"
    )
